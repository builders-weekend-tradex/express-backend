...
import { getLexiChat, getMoreInformation } from "./apiCalls/lexi.js";
...
app.post("/getMoreInformation", async (req, res) => {
  const { tickerSymbol } = req.body;
  ...
  try {
    const responseFromLexi = await getMoreInformation(tickerSymbol);
    const messageToStream = removingBrackets(
      responseFromLexi.choices[0].message.content
    );
    res.status(200).json(messageToStream);
  } catch (error) {
    console.error("Error calling getMoreInformation:", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
